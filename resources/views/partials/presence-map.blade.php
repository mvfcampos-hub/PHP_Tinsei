    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20 relative">
        <div class="text-center max-w-2xl mx-auto mb-12">
            ...
            <p class="text-brand-200 mt-3">
                @if ($topRegion)
                    Mais de {{ number_format($totalDevices, 0, ',', '.') }} dispositivos usando as nossas soluções, com maior concentração na região {{ $topRegion->name }} ({{ number_format($topRegion->device_count, 0, ',', '.') }} dispositivos), e também no México e nos Estados Unidos.
                @else
        ...
        <div class="grid lg:grid-cols-3 gap-8 items-start" x-data="{ hovered: null }">
            <div class="lg:col-span-2 rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
                <div class="flex items-center justify-between mb-3 min-h-[28px]">
                    ...
                    <p class="text-sm font-semibold text-white" x-show="hovered" x-cloak x-text="hovered ? (hovered.name + ' — ' + hovered.count.toLocaleString('pt-BR') + ' dispositivos') : ''"></p>
                ...
                <svg viewBox="{{ $mapData['view_box'] }}" class="w-full h-auto max-h-[420px] mx-auto">
                    @foreach ($mapData['states'] as $state)
                        ...
                        <path
                            ...
                            @mouseenter="hovered = { code: '{{ $regionCode }}', name: '{{ addslashes($regionName) }}', count: {{ $count }} }"
                            @mouseleave="hovered = null"
                        ><title>{{ $state['name'] }} ({{ $regionName }}): {{ number_format($count, 0, ',', '.') }} dispositivos</title></path>
                    @endforeach
                </svg>
                            ...
                            @foreach ($sortedRegions as $region)
                                <li>
                                    <div class="flex items-center justify-between text-sm mb-1">
                                        ...
                                        <span class="text-brand-300">{{ number_format($region->device_count, 0, ',', '.') }}</span>
                                    </div>
